...
export default function DashboardFavoritesPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 px-6 py-10">
      ...
      <div>
        <h1 className="font-display text-4xl text-ink-950">Client favorites</h1>
        <p className="mt-2 text-ink-600">
          Hearts from the public gallery pages, grouped by visitor session.
        </p>
      </div>
      {loading ? (
        <p className="text-ink-500">Loading…</p>
      ) : rows.length === 0 ? (
        <p className="text-ink-600">No favorites yet.</p>
      ) : (
